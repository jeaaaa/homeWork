    ...
    detect: function (ua) {
      ...
      if (ua.match("Windows")) {
        ...
        if (ua.match("Windows Phone OS")) {
          ...
          if (
            typeof WINDOWS_PHONE_MODELS[manufacturer] !== "undefined" &&
            typeof WINDOWS_PHONE_MODELS[manufacturer][model] !== "undefined"
          ) {
            this.device.manufacturer =
              WINDOWS_PHONE_MODELS[manufacturer][model][0];
      ...
      if (ua.match("Android")) {
        ...
        if (this.os.version >= 4 && ua.match("Mobile"))
          this.device.type = "mobile";
        ...
        if (
          ...
          (match = /\(([^;]+);U;Android\/[^;]+;[0-9]+\*[0-9]+;CTC\/2.0\)/.exec(
            ua
          ))
        ) {
          this.device.model = match[1];
        } else if (
          (match = /;\s?([^;]+);\s?[0-9]+\*[0-9]+;\s?CTC\/2.0/.exec(ua))
        ) {
          this.device.model = match[1];
        ...
        if (this.device.model) {
          ...
          if (typeof ANDROID_MODELS[model] !== "undefined") {
            ...
            if (typeof ANDROID_MODELS[model][2] !== "undefined")
              this.device.type = ANDROID_MODELS[model][2];
            this.device.identified = true;
          }
        ...
        if (!ua.match("Opera")) {
          ...
          if (typeof this.device.model !== "undefined") {
            if (typeof BLACKBERRY_MODELS[this.device.model] !== "undefined") {
              this.device.model =
                "BlackBerry " +
                BLACKBERRY_MODELS[this.device.model] +
                " " +
                this.device.model;
            } else {
      ...
      if (ua.match("[b|B]ada")) {
        ...
        if (
          ...
          typeof BADA_MODELS[this.device.manufacturer][this.device.model] !==
            "undefined"
        ) {
          var manufacturer = this.device.manufacturer;
          var model = cleanupModel(this.device.model);
      ...
      if (
        (match = /HbbTV\/1.1.1 \([^;]*;\s*([^;]*)\s*;\s*([^;]*)\s*;/.exec(ua))
      ) {
      ...
      if (!this.device.model && !this.device.manufacturer) {
        ...
        for (var i = 0; i < candidates.length; i++) {
          ...
          if (!result) {
            ...
            if (
              (match = /(?:^|\()LGE?(?:\/|-|_|\s)([^\s]*)/.exec(candidates[i]))
            ) {
              this.device.manufacturer = STRINGS_LG;
              this.device.model = cleanupModel(match[1]);
              this.device.type = "mobile";
            ...
            if (
              (match = /^SonyEricsson([^\/_]+)(?:\/|_|$)/.exec(candidates[i]))
            ) {
            ...
            if ((match = /^SAMSUNG-([^\/_]+)(?:\/|_|$)/.exec(candidates[i]))) {
              ...
              if (this.os.name === "Bada") {
                ...
                if (
                  typeof BADA_MODELS[manufacturer] !== "undefined" &&
                  typeof BADA_MODELS[manufacturer][model] !== "undefined"
                ) {
                  this.device.manufacturer =
                    BADA_MODELS[manufacturer][model][0];
                  this.device.model = BADA_MODELS[manufacturer][model][1];
                  this.device.identified = true;
                }
                ...
                if (
                  typeof TOUCHWIZ_MODELS[manufacturer] !== "undefined" &&
                  typeof TOUCHWIZ_MODELS[manufacturer][model] !== "undefined"
                ) {
                  this.device.manufacturer =
                    TOUCHWIZ_MODELS[manufacturer][model][0];
      ...
      if ((match = /Trident\/([0-9.]*)/.exec(ua))) {
        ...
        if (this.browser.name === "Internet Explorer") {
          if (
            parseVersion(this.engine.version) === 6 &&
          ...
          if (
            parseVersion(this.engine.version) === 5 &&
          ...
          if (
            parseVersion(this.engine.version) === 4 &&
      ...
      if (this.options.detectCamouflage) {
        if (
          (match = /Mac OS X 10_6_3; ([^;]+); [a-z]{2}-(?:[a-z]{2})?\)/.exec(
            ua
          ))
        ) {
          this.browser.name = "";
        ...
        if (this.browser.name === "Safari") {
          if (
            this.os.name !== "iOS" &&
            /AppleWebKit\/([0-9]+.[0-9]+)/i.exec(ua)[1] !==
              /Safari\/([0-9]+.[0-9]+)/i.exec(ua)[1]
          ) {
